...
def cleanup(vs):
    ...
    vs.stop()
# morse_code_mapping = {".": "0", "-": "1", "/": "", "//": " "}
# def translate_morse_code(morse_sequence):
#     translated_text = ""
#     current_word = ""
    
#     for char in morse_sequence:
#         if char == '/':
#             translated_text += morse_code_mapping.get(current_word, "")
#             current_word = ""
#         elif char == '//':
#             translated_text += morse_code_mapping.get(current_word, "")
#             translated_text += " "
#             current_word = ""
#         else:
#             current_word += MORSE_REVERSED.get(char, "")
...
def print_results(total_morse):
	...
	# translated_text = translate_morse_code(total_morse)
	# print("Translated Text:", translated_text)
	morse_sequence = total_morse
...
# Convert Morse code sequence to its numerical representation
	...
	morse_encoded += [2] * (6 - len(morse_encoded))
	predicted_text_encoded = model.predict([morse_encoded])
	print(predicted_text_encoded)
	predicted_text = label_encoder.inverse_transform(predicted_text_encoded)
...
# # Assuming you have a pre-trained Random Forest model saved as 'rf_morse_model.pkl'
# # and a LabelEncoder for Morse code saved as 'morse_label_encoder.pkl'
# model = joblib.load('rf_morse_model.pkl')
# label_encoder = joblib.load('morse_label_encoder.pkl')
...
# def interpret_blinks_as_morse(COUNTER, CLOSED_EYES):
#     # Simplified interpretation: short blink = ".", long blink = "-"
#     if CLOSED_EYES:
#         if COUNTER < 5:  # Threshold for short/long blink can be adjusted
#             return "."
#         else:
#             return "-"
#     return ""
...
#             if eye_ar < 0.2:  # Threshold for blink detection
#                 COUNTER += 1
#                 CLOSED_EYES = True
#             else:
#                 if CLOSED_EYES:
#                     morse_code = interpret_blinks_as_morse(COUNTER, CLOSED_EYES)
#                     morse_sequence.append(morse_code)
#                     COUNTER = 0
#                     CLOSED_EYES = False
#         cv2.imshow("Frame", frame)
#         key = cv2.waitKey(1) & 0xFF
#         if key == ord("]"):  # Exit condition
#             break

#     morse_encoded = label_encoder.transform(morse_sequence)
#     predicted_text_encoded = model.predict([morse_encoded])
#     predicted_text = label_encoder.inverse_transform(predicted_text_encoded)
#     print("Predicted Text:", "".join(predicted_text))
#     cv2.destroyAllWindows()
